...
cross-compile a pacman-based base system with glibc for a new architecture.

Steps for introducing new architecture include:
- adding the compiler triplet and arch type to makepkg.conf
- adjusting build rules for packages that are arch aware:
  gcc, openssl, linux-api-headers, glibc
- create new kernel config for linux
- optionally configure pacman architecture
